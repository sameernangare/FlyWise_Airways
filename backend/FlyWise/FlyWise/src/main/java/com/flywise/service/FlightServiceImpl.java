...
//--------------------------------------------------------------------------------------------------------------------------
	...
	public List<Passenger> getPassengersByFlightId(int flightId) {
		...
		List<Passenger> listOfPassengers = flight.getListOfPassengers();

		return listOfPassengers.stream()
								.filter(c -> c.getBooking().getBookingStatus() == 1)
								.collect(Collectors.toList());
		
//		List<Passenger> listOfUsers = passengerRepo.findAll();
//	
//	return	listOfUsers.stream()
//						.filter(c->(c.getFlight().getFlightId() == flightId) && c.getBooking().getBookingStatus() == 1)
//						.collect(Collectors.toList());
	}
...
//--------------------------------------------------------------------------------------------------------------------------
	...
	@Override
	public List<Flight> fetchFlightsWithCondition(String source, String destination, LocalDate travelDate) throws FlightException {
